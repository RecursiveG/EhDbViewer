...
const QString DataImporter::kNoImageBase64 =
    ...
    "AP+"
    "gvaeTAAAEJElEQVR4nO3dv49MURTA8e8Ku4kIiUKCRKHRaFQqlYqCjdiQkCgp7Cr5Eyh1dHTb6VSikbAaK1HQiFixiSAhg8KuXcWbiTFz38977jtnZ"
    ...
    "oa9fUhBwlLolBwFHqVMrIOAoVWsNBBylSq2CgKOU1ToIOEpRKiDgKHmpgYCjhFIFAUcZTB0EHKU/EyDgKL3MgED2p0RW/8RoBlgAloAf3W0JmO/"
    "+TipTIGATZT/wkuHH3tuWu7eRyBwI2EKZoRijH0XiSDEJAnZQFijH6G1XBe7PLAjYQHlOdZBnAvdnGgT0X311qA7SEbg/8yCgi1IH5LvA/"
    ...
    "S1IrYFcBv4E9r9ZY4wmSRwpcwNjpDzSWgG5DmwM7LcBXKs52abFoAxipEZJCjJFdgQM7rMGXGo238Y1QcnDSImSDGQLcDdw+"
    "19kF3aN6lxT8m6beu0sCcg0/6/Y9rYOcDxuvtFVOVJCR8Y6cKHi/"
    "jGJg+St2H4FjsbPV6SiJ7UIo8r+sYmC5K3YrpAtk1gq9pSUaulfDCR2xVajsi83GDwyquwfe6SIgEit2GqUh1KGUbR/"
    ...
    "wi2LZSJAZGojVdfDlKz1CgO0qCUKA7SsFTXFAeJKMWR4iCRpUYpzUGGS4lSmoOES4VSmoPkJ3GhdxDh1D8P8YZT/TzEC9cUJRrEt7T/qU1p2g/"
    "e6pYKpTTtB255S4FSmvaDtr5Jo3gJG9V/"
    "iDrWOYrBHMVgjmIwRzGYoxgsb5X4tOakJr0QyorqjLwhlFXd6XiQnabekR0dp5Tn4nme541RfwFbbDokN3PzagAAAABJRU5ErkJggg==";
...
std::optional<QString> DataImporter::ScanFolder(const QDir &dir, std::vector<FolderData> *output_list,
                                                QProgressDialog *progress) {
    // qDebug() << "scanning folder " << dir.path();
    ...
    //        return QString("Unable to process folder containing both picture and subfolder:
    //        %1").arg(dir.absolutePath());
    //    }

    if (pic_info) {
        output_list->push_back({dir, pic_info->fileName(), ""});
...
QString DataImporter::ImportDir(QDir dir, QWidget *parent) {
    ...
    auto transaction_msg = EhDbViewerDataStore::DbTransaction([dir, parent, &ret](QSqlDatabase *db) -> bool {
        ...
        discovered_folders.erase(std::remove_if(discovered_folders.begin(), discovered_folders.end(),
        ...
        for (size_t idx = 0; idx < discovered_folders.size(); idx++) {
            ...
            folder.thumb_base64 = GenerateImgThumbnail(folder.folder.filePath(folder.cover_fname));
            progress.setLabelText(QString("Generating thumbnail: %1").arg(folder.folder.dirName()));
        ...
        for (const auto &folder : discovered_folders) {
            ...
            if (!EhDbViewerDataStore::DbInsert(
                    *db, schema::CoverImages{
                             ...
                             .cover_base64 = (folder.thumb_base64.size() == 0 ? kNoImageBase64 : folder.thumb_base64),
                         })) {
                ret = "Error: failed to insert to db";
                return false;
            }
        }
        ret = QString("Import complete: %1 new folders imported").arg(discovered_folders.size());
        return true;
    });
...
void DataImporter::SelectThumbnailInFolder(QString dir, QString *filename_out, QString *filepath_out) {
    *filename_out = "";
    *filepath_out = "";
    for (const auto &info : QDir{dir}.entryInfoList(QDir::NoDotAndDotDot | QDir::AllEntries)) {
        if (info.isFile()) {
            auto ext = info.suffix().toLower();
            if (ext == "jpg" || ext == "png" || ext == "jpeg") {
...
QString DataImporter::ImportEhViewerBackup(QString db_file, QDir download_dir, QWidget *parent) {
    ...
    auto transaction_err = EhDbViewerDataStore::DbTransaction([parent, &ret, &ehdb,
        ...
        for (schema::EhBackupImport &eh_data : new_list) {
            ...
            QDir dir{download_dir.filePath(QString::fromStdString(eh_data.dirname))};
            if (!dir.exists()) {
                ret = QString("Dir %1 not exists").arg(QString::fromStdString(eh_data.dirname));
                return false;
            }
            ...
            progress.setLabelText(QString("Importing: %1").arg(QString::fromStdString(title)));
